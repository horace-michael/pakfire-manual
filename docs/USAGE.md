...
in the sidebar. The page has four sections:
...
| **Core Addon Matrix** | `core_addon_matrix.tsv` | Per-core add-on version table sourced from IPFire blog posts. The planner reads this to report which add-ons change at each step. |
...
The WUI is the primary interface for managing data files — no SSH required for
day-to-day maintenance. The CLI tools are for running the survey and the planner.

![Pakfire Manual WUI — v1.0.0](wui-overview-v1.0.0.png)
