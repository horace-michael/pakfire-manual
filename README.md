...
[firewall-local](https://codeberg.org/H-ipfire/firewall-local) add-on
...
├── utilities/
│   └── make-package.sh              builds the .ipfire package + SHA256
└── docs/
...
- `curl` and `wget`
- GPG keyring containing the IPFire project signing key
- `firewall-local` ≥ 1.8.0 recommended (provides customlocationgrp)
...
```bash
# On IPFire as root — fetches from GitHub, verifies SHA256, extracts, installs:
...
```bash
# On a workstation — download the release asset:
wget https://github.com/horace-michael/pakfire-manual/releases/download/v1.0.1/pakfire-manual-v1.0.1.ipfire

# Copy to IPFire:
scp pakfire-manual-v1.0.1.ipfire root@<ipfire-host>:/opt/pakfire/tmp/

# On IPFire as root:
cd /opt/pakfire/tmp
tar -xvf pakfire-manual-v1.0.1.ipfire
NAME=pakfire-manual bash install.sh
```
